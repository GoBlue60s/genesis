...
class AlikeCommand:
	...
	def _update_coordinates_for_similar_pairs(
			self, filtered_pairs: list[list]) -> None:
		...
		for pair in filtered_pairs:
			...
			item_a_index = self._director.configuration_active.point_labels.\
				index(item_a_label)
			item_b_index = self._director.configuration_active.point_labels.\
				index(item_b_label)
	...
	def _print_alike_pairs(self) -> None:
		...
		for each_pair in range(len(most_alike_pairs)):
			print(f"\t {most_alike_pairs[each_pair][3]:<15}"
...
# ------------------------------------------------------------------------
	...
	def _get_cutoff_from_user(self) -> None:
		...
		cutoff_dialog = SetValueDialog(
			title, label, min_allowed, max_allowed,
			an_integer, default)
...
# --------------------------------------------------------------------------
	...
	def _create_alike_table(self) -> pd.DataFrame:
		...
		for pair in sorted_similarities_w_pairs:
			...
			if (value_type == "similarities" and similarity > cut_point) \
				or (value_type == "dissimilarities"
					and similarity < cut_point):
		...
		alike_df = pd.DataFrame(
			alike_pairs, columns=[
				"Item", "Paired with", similarity_column_name])
...
class LineOfSightCommand:
	...
	def execute(
		...
		self._director.dependency_checker.detect_dependency_problems()
		self._director.similarities_candidate = \
			self.common.los(self._director.evaluations_active)
...
class PairedCommand:
	...
	def _print_paired(self) -> None:
		...
		print(f"  {display_df.columns[0]:<20} {sim_label:>12} "
			f"{display_df.columns[2]:>12}")
		print("  " + "-" * 48)
		for _, row in display_df.iterrows():
			print("  {:<20} {:>12.4f} {:>12.4f}".format(
				row["Name"],
				row["Similarity"],
				row["Distance"]
			))
...
class ScreeCommand:
	...
	def __init__(
		...
		self._min_stress: pd.DataFrame = self.\
			_director.configuration_active.min_stress
...
# ------------------------------------------------------------------------
	...
	def _scree(self) -> None:
		...
		for each_n_comp in range_ncomps:
			...
			print(
				f"\tBest stress in {each_n_comp} dimensions: "
				f"{best_stress:.4f}")
...
class StressContributionCommand:
	...
	def _calculate_and_sort_stress_contributions(self) -> list[float]:
		...
		ranks_df["Pct_of_Stress"] = \
			(ranks_df["Pct_of_Stress"] / total_differences) * 100
		print(f"\n\nDEBUG -- nxt..."
			f"\n{ranks_df['Pct_of_Stress'].sum()=}")
		sorted_dyads = ranks_df.sort_values(by='Absolute_Difference',
			ascending=False)
		worst_fit = sorted_dyads[
			["A_label", "B_label", "Similarity_Rank", "Distance_Rank",
				"Pct_of_Stress"]]
...
# --------------------------------------------------------------------------
	...
	def _create_stress_contribution_df(
			self, index: int, worst_fit: pd.DataFrame) -> pd.DataFrame:
		"""
