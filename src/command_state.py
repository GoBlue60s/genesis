...
class CommandState:
	...
	def capture_correlations_state(self, director: Status) -> None:
		...
		self.state_snapshot["correlations"] = {
			# Core data - the raw lower triangular list
			"correlations": [row.copy() for row in corr.correlations]
			...
			"nitem": corr.nitem,
			"nreferent": corr.nreferent,
			"ncorrelations": corr.ncorrelations,
			# Note: Derived structures (correlations_as_dataframe,
			# correlations_as_dict, etc.) are NOT captured here.
			# They will be regenerated by duplicate_correlations()
			# during restoration.
		}
	...
	def capture_similarities_state(self, director: Status) -> None:
		...
		self.state_snapshot["similarities"] = {
			...
			# Note: Derived structures (similarities_as_dataframe,
			# similarities_as_dict, etc.) are NOT captured here.
			# They will be regenerated by duplicate_similarities()
			# during restoration.
		}
	...
	def restore_configuration_state(self, director: Status) -> None:
		...
		print("\tRestoring configuration state:")
		coords_empty = config_snapshot['point_coords'].empty
		coords_shape = config_snapshot['point_coords'].shape
		print(f"\t  point_coords.empty: {coords_empty}")
		print(f"\t  point_coords.shape: {coords_shape}")
	...
	def restore_correlations_state(self, director: Status) -> None:
		...
		Restores the raw correlation data and metadata, then regenerates
		all derived structures (dataframes, dicts, etc.) by calling
		...
		# Restore core data
		corr.correlations = [
			row.copy() for row in corr_snapshot["correlations"]
		]
		# Restore metadata
	...
	def restore_similarities_state(self, director: Status) -> None:
		...
		Restores the raw similarities data and metadata, then regenerates
		all derived structures (dataframes, dicts, etc.) by calling
		...
		# Regenerate all derived structures
		sims.duplicate_similarities(common)
