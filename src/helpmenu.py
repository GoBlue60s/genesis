...
class AboutCommand:
	...
	def __init__(
		...
		self._director.acknowledgements = (
			"Charles Antonelli", "Ned Batchelder", "Cornell Belcher",
			"Alexis Castelanas", "ChatGPT", "Claude", "Clyde Coombs",
			"Beautiful Corners", "Meghan Dailey", "Anthony Downs",
			"Lutz Erbring", "Chris Gates", "James Gerity", "GitHub Copilot",
			"Jonathan Golob", "Peter Heller", "Geir Arne Hjelle",
			"Inter-Univerity Consortium for\nPolitical and Social Research",
			"Institute for Social Research,\nThe University of Michigan",
			"William Jacoby", "Jon Kiparsky", "William Kruskal",
			"Celinda Lake", "Lake Research Partners", "Glenn Lehman",
			"Adam Masiarek", "Stuart McDonald", "Gijs Mos",
			"Blaise Pabon", "George Rabinowitz", "Real Python",
			"Roger Shepard", "Donald Stokes", "Warren Tobler",
			"Ruud van der Ham", "Herbert Weisberg", "Bartosz Zaczynski"
		)
		...
		if n_names <= MAXIMUM_NUMBER_OF_ROWS_IN_ACKNOWLEDGEMENTS_TABLE:
			...
			for each_col in range(columns_required):
				for each_row in range(1, rows_required):
					...
					if index < n_names:
						table_widget.setItem(
							each_row, each_col, QTableWidgetItem(str(
								names[index])))
		return table_widget
...
class StatusCommand:
	...
	def _create_table_widget_for_status(self) -> QTableWidget:
		...
		#
		table_widget.setItem(
			0, 0, QTableWidgetItem("Active configuration"))
		table_widget.setItem(
			0, 1, QTableWidgetItem(
				str(self._director.common.have_active_configuration())))
		table_widget.setItem(
			1, 0, QTableWidgetItem("Alike coordinates"))
		table_widget.setItem(
			1, 1, QTableWidgetItem(
				str(self._director.common.have_alike_coords())))
		# table_widget.setItem(
		...
		table_widget.setItem(
			2, 0, QTableWidgetItem("Clusters"))
		table_widget.setItem(
			2, 1, QTableWidgetItem(
				str(self._director.common.have_clusters())))
		table_widget.setItem(
			3, 0, QTableWidgetItem("Correlations"))
		table_widget.setItem(
			3, 1, QTableWidgetItem(
				str(self._director.common.have_correlations())))
		table_widget.setItem(
			4, 0, QTableWidgetItem("Data for groups"))
		table_widget.setItem(
			4, 1, QTableWidgetItem(
				str(self._director.common.have_grouped_data())))
		table_widget.setItem(
			5, 0, QTableWidgetItem("Data for individuals"))
		table_widget.setItem(
			5, 1, QTableWidgetItem(
				str(self._director.common.have_individual_data())))
		table_widget.setItem(
			6, 0, QTableWidgetItem("Distances"))
		table_widget.setItem(
			6, 1, QTableWidgetItem(
				str(self._director.common.have_distances())))
		table_widget.setItem(
			7, 0, QTableWidgetItem("Evaluations"))
		table_widget.setItem(
			7, 1, QTableWidgetItem(
				str(self._director.common.have_evaluations())))
		table_widget.setItem(
			8, 0, QTableWidgetItem("Factors"))
		table_widget.setItem(
			8, 1, QTableWidgetItem(
				str(self._director.common.have_factors())))
		table_widget.setItem(
			9, 0, QTableWidgetItem("MDS results"))
		table_widget.setItem(
			9, 1, QTableWidgetItem(
				str(self._director.common.have_mds_results())))
		table_widget.setItem(
			10, 0, QTableWidgetItem(
				"Previous active configuration"))
		table_widget.setItem(
			10, 1, QTableWidgetItem(
				str(self._director.common.have_previous_active())))
		table_widget.setItem(
			11, 0, QTableWidgetItem("Ranks"))
		table_widget.setItem(
			11, 1, QTableWidgetItem(
				str(self._director.common.have_ranks_of_similarities())))
		table_widget.setItem(
			12, 0, QTableWidgetItem("Reference points"))
		table_widget.setItem(
			12, 1, QTableWidgetItem(
				str(self._director.common.have_reference_points())))
		table_widget.setItem(
			13, 0, QTableWidgetItem("Scores"))
		table_widget.setItem(
			13, 1,
			QTableWidgetItem(str(self._director.common.have_scores())))
		table_widget.setItem(
			14, 0, QTableWidgetItem("Segments"))
		table_widget.setItem(
			14, 1,
			QTableWidgetItem(str(self._director.common.have_segments())))
		table_widget.setItem(
			15, 0, QTableWidgetItem("Similarities"))
		table_widget.setItem(
			15, 1,
			QTableWidgetItem(str(self._director.common.have_similarities())))
		table_widget.setItem(
			16, 0, QTableWidgetItem("Target configuration"))
		table_widget.setItem(
			16, 1,
			QTableWidgetItem(\
		...
		table_widget.setItem(
			17, 0,
			QTableWidgetItem("Commands include explanations"))
		table_widget.setItem(
			17, 1,
...
class VerboseCommand:
	"""The Verbose command toggles the include_explanation indicator to True.
	"""
	def __init__(
			self,
			director: Status,
