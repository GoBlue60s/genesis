...
class DataFrameViewer(QWidget):
	...
	def init_ui(self) -> None:
		...
		self.table_widget.setHorizontalHeaderLabels(
			map(str, self.dataframe.columns)
		)
		self.table_widget.setVerticalHeaderLabels(
			map(str, self.dataframe.index)
		)
...
class Item:
	def __init__(
		self, name: str = "", label: str = "", location: Point = None
	) -> None:
...
class ItemFrame:
	...
	def __init__(
		...
		self._columns = (
			columns if columns is not None else ["name", "label", "location"]
		)
		self._dim_names = dim_names if dim_names is not None else ["x", "y"]
		self._dim_labels = dim_labels if dim_labels is not None else ["X", "Y"]
		...
		if isinstance(data, dict):
			...
			for item_index, row in enumerate(data):
				idx = (
					index[item_index]
					if index and item_index < len(index)
					else item_index
				)
	...
	def _create_item_from_row(self, row: int) -> Item:
		...
		if len(row) >= TEST_FOR_MIN_ROW_LENGTH and isinstance(
			row[INDEX_OF_LOCATION_IN_ROW], Point
		):
			item = Item(
	...
	def dim_1_coords(self) -> list[float]:
		...
		dim_name = (
			self._dim_names[INDEX_OF_FIRST_DIMENSION_NAME_IN_DIM_NAMES]
			if self._dim_names
			else "x"
		)
	...
	def dim_2_coords(self) -> list[float]:
		...
		if len(self._dim_names) < TEST_FOR_LESS_THAN_TWO_COORDINATES:
			return []
		dim_name = self._dim_names[INDEX_OF_SECOND_DIMENSION_NAME_IN_DIM_NAMES]
		coords = self.get_dimension_coords(dim_name)
		return coords
	...
	def __str__(self) -> str:
		...
		name_width = max(
			[len(item.name) for item in self._data.values()] + [4]
		)
	...
	def full(self, *, show_headers: bool = True) -> str:
		...
		label_width = max(
			[len(item.label) for item in self._data.values()] + [5]
		)
		coord_width = 10  # Fixed width for coordinates
