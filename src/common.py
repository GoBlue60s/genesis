...
class Spaces:
	def __init__(
		...
		self.displacement: float = .04  # to improve visibility

		self.vector_head_width: float = 0.05
		self.vector_width: float = .01
		self.battleground_size: float = .25
	...
	def create_sample_design_analysis_table(self) -> pd.DataFrame:
		...
		sample_design_analysis_df = pd.DataFrame(columns=[
			"Repetition",
			"Selected Count",
			"Selected Percent",
			"Not Selected Count",
		...
		for repetition in range(1, number_of_repetitions + 1):
			...
			selected_row = rep_data[
				rep_data["Selected"] == True] # noqa: E712
			not_selected_row = rep_data[
				rep_data["Selected"] == False] # noqa: E712
			
			# Extract counts
			selected_count = selected_row["Count"].to_numpy()[0] \
				if not selected_row.empty else 0
			not_selected_count = not_selected_row["Count"].to_numpy()[0] \
				if not not_selected_row.empty else 0
			
			# Calculate total and percentages
			total = selected_count + not_selected_count
			selected_percent = (selected_count / total * 100) \
				if total > 0 else 0
			not_selected_percent = (not_selected_count / total * 100) \
				if total > 0 else 0
...
# ------------------------------------------------------------------------
	...
	def create_uncertainty_table(self) -> pd.DataFrame:

		# Get stress data from sample_repetitions_stress
		...
		# 	# Add a row with repetition number (index + 1) and stress value
		# 	uncertainty_analysis_df.loc[len(uncertainty_analysis_df)] = [
		# 		index + 1,
		# 		stress_value
		# 	]

		# Create DataFrame directly with repetition numbers as integers
		data = {
			"Repetition": list(range(1, len(sample_repetitions_stress) + 1)),
			"Stress": sample_repetitions_stress
		}
...
# ------------------------------------------------------------------------
	...
	def get_focal_item_from_user(
		...
		if dialog.exec() == QDialog.Accepted:
			...
			raise SpacesError(
				self.no_selection_error_title,
				self.no_selection_error_message)
...
# --------------------------------------------------------------------------
	...
	def set_axis_extremes_based_on_coordinates(
		...
		extremes.
		"""
		if self._director.common.have_scores() and \
			self._director.scores_active.hor_max == 0.0 \
			and self._director.scores_active.hor_min == 0.0:


			numpy_scores = self._director.scores_active. \
				scores.iloc[:, 1:3].to_numpy()
			...
			self._director.scores_active.offset = .03 * score_int_max
		...
		self._director.common.plot_ranges = \
			PlotExtremes(hor_max, hor_min, vert_max, vert_min, offset)
...
# --------------------------------------------------------------------------
	...
	def read_configuration_type_file(
			self,
			file_name: str,
			configuration_type: str) -> ConfigurationFeature | TargetFeature:
		...
		try:
			...
			raise SpacesError(
				...
				self.conf_file_not_found_message) from None
...
# ------------------------------------------------------------------------
	...
	def read_configuration_type_file_sizes_initialize_variables(
			self, file_handle: TextIO, configuration_type: str) -> None:
	...
	def read_configuration_type_file_sizes(
			self,
			file_handle: TextIO,
			configuration_type: str) -> tuple[int, int]:
		...
		try:
			...
			if len(header) == 0:
				raise SpacesError(
					...
					self.empty_header_error_message) from None
			if header.lower().strip() != "configuration":
				raise SpacesError(
					self.not_configuration_file_error_title,
					self.not_configuration_file_error_message) from None
			...
			raise SpacesError(
				self.not_found_error_title,
				self.not_found_error_message) from err
...
# --------------------------------------------------------------------------
	...
	def read_configuration_type_file_coordinates(
		self, file_handle: TextIO, npoint: int, point_names: list[str],
		ndim: int, dim_labels: list[str]) -> pd.DataFrame:
		...
		try:
			...
			for i in range(npoint): # noqa: B007
				...
				try:
					...
					raise SpacesError(
						...
						self.invalid_coordinate_error_message) from err
			...
			raise SpacesError(
				...
				self.unable_to_read_coordinates_error_message) from e
...
# ----------------------------------------------------------------------
	...
	def create_plot_for_plot_and_gallery_tabs(self, plot_type: str) -> None:
		...
		if self._director.common.presentation_layer == "PyQtGraph":
			try:
				getattr(self._director.pyqtgraph_plotter,
					f"request_{plot_type}_plot_for_plot_and_gallery_tabs_using_pyqtgraph")()
			except UnknownTypeError:
				getattr(self._director.matplotlib_plotter,
	...
	def duplicate_in_different_structures(
		...
		for an_item in from_points:
			...
			for another_item in to_points:
				values_as_list.append(
					values[an_item - 1][another_item])
				new_key = str(
					item_labels[another_item]
					+ "_" + item_labels[an_item])
		...
		if value_type == "similarities":
			...
			sorted_values_w_pairs = sorted(zip(
				values_as_list,
				a_item_label, b_item_label,
				a_item_name, b_item_name, strict=True),
				reverse=True)
		else:
			...
			for other_item in range_items:
				if each_item == other_item:
					...
					index = str(
						...
						+ "_" + item_labels[other_item])

					values_as_square[each_item].append(
						values_as_dict[index])
				else:
					index = str(
						item_labels[other_item]
						+ "_" + item_labels[each_item])
					values_as_square[each_item].append(
						values_as_dict[index])
		...
		return (
			values_as_dataframe, values_as_dict, values_as_list,
			values_as_square, sorted_values_w_pairs, ndyad,
			range_dyads, range_items)
	...
	def have_reference_points(self) -> bool:
		...
		return not (
			rivalry.rival_a.index is None
			or rivalry.rival_b.index is None
		)
...
# ------------------------------------------------------------------------
	...
	def have_sample_design_frequencies(self) -> bool:
		...
		return not \
			self._director.uncertainty_active.sample_design_frequencies.empty

	# ------------------------------------------------------------------------

	def have_sample_repetitions(self) -> bool:

		return not self._director.uncertainty_active.sample_repetitions.empty
	...
	def los(
			self,
			evaluations: pd.DataFrame) -> SimilaritiesFeature:
		...
		line_of_sight.range_similarities = (
			range(len(line_of_sight.item_names)))
		#
		...
		line_of_sight.n_pairs = (
			int(line_of_sight.nreferent * n_items_less_one / 2))
		line_of_sight.range_similarities = (
			range(line_of_sight.n_pairs))
		...
		for each_pair in line_of_sight.range_similarities:
			cum_b_hat[str(diffs_d_star.columns[each_pair])] = combo_b[
				str(diffs_d_star.columns[each_pair])].cumsum(axis=0)
		...
		for each_row in range_rows:
			rho = spearmanr(
				ordered.iloc[each_row],
				ordered.iloc[each_row - 1])[0]
			unique_vals = ordered.iloc[each_row].nunique()
			discrim = (unique_vals - 1) / line_of_sight.n_pairs
			dense = \
				(evaluations.nevaluators - each_row) / evaluations.nevaluators
...
# --------------------------------------------------------------------------
	...
	def mds(
			...
			similarities: SimilaritiesFeature) -> ConfigurationFeature:
		...
		npos = nmds.fit_transform(
			X=similarities.similarities_as_square)
		...
		configuration.point_coords.columns \
			= configuration.dim_names
...
# ---------------------------------------------------------------------------
	...
	def needs_sample_design_frequencies(
			self,
			command: str) -> bool:
	...
	def needs_target(
		...
		if not self.have_target_configuration():
			...
			message = (
				"Open a target file to establish one before using "
				f"{command}."
			)
			raise DependencyError(title, message)
	...
	def print_established_elements(self) -> None:
		...
		print("\t Alike coordinates: ",
			self._director.common.have_alike_coords())
		print(
			"\t Configuration: ",
		...
		print(
			"\t Verbosity setting: ",
			self._director.\
				include_explanation_when_verbosity_last_set_to_verbose())
		print(" ")
		return
	...
	def print_lower_triangle(
		decimals: int, labels: list[str], names: list[str], nelements: int,
		values: list[float], width: int) -> None:
...
# ------------------------------------------------------------------------
	...
	def read_lower_triangular_matrix_initialize_variables(
			self) -> None:
	...
	def read_lower_triangular_matrix(
		...
		try:
			...
			raise SpacesError(
				self.unexpected_eof_lower_triangular_error_title,
				self.unexpected_eof_lower_triangular_error_message) from None
		except OSError:
			raise SpacesError(
				self.problem_reading_lower_triangular_error_title,
				self.problem_reading_lower_triangular_error_message) \
...
# ------------------------------------------------------------------------
	...
	def read_lower_triangle_size(
			self,
			file_handle: TextIO) -> int:
...
# ------------------------------------------------------------------------
	...
	def read_lower_triangle_dictionary(
			self,
			file_handle: TextIO,
			nreferent: int) -> tuple[list[str], list[str]]:
		item_labels: list = []
		item_names: list = []
		docs: list = []
...
# ------------------------------------------------------------------------
	...
	def read_lower_triangle_values(
		...
		for each_item in nitems:
			...
			for ites in range_ites:
				values[each_item][ites] \
					= float(values[each_item][ites])
		return values
	...
	def write_a_label_for_each_point(
			self,
			file_handle: TextIO, nreferent: int,
			item_labels: list[str], item_names: list[str]) -> None:
		
		for each_item in range(nreferent):
			file_handle.write(
				f"{item_labels[each_item]};"
				f"{item_names[each_item]}")
	...
	def write_data_as_lower_triangle(
		self, file_handle: TextIO, data_as_list: list[float], nreferent: int,
		width: int, decimals: int) -> None:
...
# ------------------------------------------------------------------------
	...
	def write_lower_triangle(
		self, file_name: str, data_as_list: list[float], nreferent: int,
		item_labels: list[str], item_names: list[str],
		width: int, decimals: int) -> None:
		...
		try:
			with Path(file_name).open('w') as file_handle:
				...
				self.write_a_label_for_each_point(file_handle,
					nreferent, item_labels, item_names)
			...
			raise SpacesError(
				...
				self.file_exists_lower_triangle_error_message) from exc
		return
