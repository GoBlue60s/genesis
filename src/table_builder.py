...
class BuildOutputForGUI(QWidget):
	def __init__(
		...
		if director.command in director.widget_dict:
			...
			print(f"Key: {director.command=} "
				f"not found in the widget dictionary")
...
class BasicTableWidget:
	...
	def _build_basic_table(
			self, data: pd.DataFrame, row_headers: list[str],
			column_headers: list[str], row_height: int,
			format_str: str) -> QTableWidget:
...
class SquareTableWidget:
	...
	def display_table(self, table_name: str) -> QTableWidget:
		...
		result = self._build_square_table(
			...
			row_height, format_str, diagonal)
	...
	def _build_square_table(
			...
			row_height: int, format_str: str, diagonal: str) -> QTableWidget:
		...
		try:
			...
			self._director.set_column_and_row_headers(
				table_widget, column_headers, row_headers)
			self._director.resize_and_set_table_size(
				table_widget, row_height)
			self._director.output_widget_type = "Table"
			
		except AttributeError as e:
			print(f"Cannot display square table:"
...
class StatisticalTableWidget:
	def __init__(self, director: Status) -> None:
		self._director = director
		self.unknown_statiscal_table_error_title = "Statistical Table Widget"
	...
	def display_table(self, table_name: str) -> QTableWidget:
		...
		match table_name:
			...
			case "factor_analysis":
				...
				column_headers = [
					...
					"Commonalities", "Uniquenesses", "Item",
					"Loadings\nFactor 1", "Loadings\nFactor 2"
				]
				row_height = 4
				format_spec = [
					"8.4f", "8.4f", "8.4f", "8.4f", "s", "8.4f", "8.4f"]
			...
			case "sample_design":
				...
				row_headers = []
				column_headers = ["Repetition", "Selected \n N", "Percent",
					"Not selected \n N", "Percent"]
				row_height = 4
			...
			case "second":
				...
				if self._director.common.have_scores():
					...
					noscores = True
				row_headers = []
				column_headers = ["Social Oriented\nSupporters of:",
					"Percent of\nPopulation"]
				row_height = 4
				format_spec = "8.1f"
		...
		result = self._build_statistical_table(
			data, row_headers, column_headers, row_height,
			format_spec, noscores)
...
# ...existing code...
	def _build_statistical_table(
		...
		noscores: bool = False) -> QTableWidget: # noqa: FBT001, FBT002
